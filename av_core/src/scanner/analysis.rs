...
fn check_file(
    ...
    md5hash: String,
    sha256hash: String,
) -> Result<Option<MalwareDB>, anyhow::Error> {
    let mut stmt =
    ...
    let mut malware_iter = stmt.query_map([md5hash], |row| {
        ...
        let malware_name: String = row.get(2)?;
        Ok(MalwareDB {
            malware_name,
            md5hash,
            sha256hash,
...
pub fn scan_path(conn: &Connection, path: &PathBuf) -> anyhow::Result<Vec<ScanResult>> {
    ...
    if path.is_file() {
        ...
        match check_file(conn, md5hash.clone(), sha256hash.clone()) {
            Ok(Some(malware)) => {
                pb.println(format!(
                    "Threat detected in {}: {} (MD5: {}, SHA256: {})",
                    path_display, malware.malware_name, malware.md5hash, malware.sha256hash
                ));
                ...
                pb.println(format!(
                    ...
                    malware.malware_name, malware.md5hash, malware.sha256hash
                ));
